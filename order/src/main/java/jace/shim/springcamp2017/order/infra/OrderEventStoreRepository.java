...
public class OrderEventStoreRepository {
	...
	public long save(OrderRawEvent commonRawEvent) {
	...
	public List<OrderRawEvent> findByIdentifier(Long identifier) {
		StringBuilder query = new StringBuilder();
	...
	public List<OrderRawEvent> findAll() {
		StringBuilder query = new StringBuilder();
		query.append(selectQuery());

		// identifier, type, version, payload, created
		return jdbcTemplate.query(query.toString(), new CommonRawEventRowMapper());
	}

	public List<OrderRawEvent> findByIdentifierAndVersionGreaterThan(Long identifier, Long version) {
		StringBuilder query = new StringBuilder();
		query.append(selectQuery());
		query.append(" WHERE identifier = ?");
	...
	static class CommonRawEventRowMapper implements RowMapper<OrderRawEvent> {

		@Override
		public OrderRawEvent mapRow(ResultSet rs, int rowNum) throws SQLException {
			final OrderRawEvent commonRawEvent = new OrderRawEvent();
